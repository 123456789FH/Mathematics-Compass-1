...
export class SupabaseBackend{
  ...
  async signIn(email,password){const {data,error}=await this.client.auth.signInWithPassword({email,password});unwrap(data,error);return{session:data.session,profile:await this.getProfile()}}
  async signUp(name,email,password){const {data,error}=await this.client.auth.signUp({email,password,options:{data:{full_name:name}}});unwrap(data,error);return{session:data.session,profile:data.session?await this.getProfile():null,message:data.session?'تم إنشاء الحساب.':'تم إنشاء الحساب. افحصي بريدك لتأكيده قبل الدخول.'}}
  async resetPassword(email){const {error}=await this.client.auth.resetPasswordForEmail(email,{redirectTo:location.origin+location.pathname});unwrap(true,error);return{message:'أُرسل رابط استعادة كلمة المرور إلى البريد.'}}
  async updatePassword(password){const {data,error}=await this.client.auth.updateUser({password});unwrap(data,error);return{message:'تم تحديث كلمة المرور بنجاح.'}}
  ...
  async adminListProfiles(){const {data,error}=await this.client.from('profiles').select('id,full_name,email,role,active,last_seen_at,created_at').order('created_at',{ascending:false});return unwrap(data,error)}
